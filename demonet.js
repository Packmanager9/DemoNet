...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Perceptron {
        ...
        weight(link) {
            let weight = new Weight(this, link)
    ...
    class Network {
        ...
        adjustWeights() {
            for (let t = 0; t <= this.structure.length - 1; t++) {
                const layer = this.structure[t]
                for (let k = 0; k < layer.length; k++) {
                    const perceptron = layer[k]
    ...
    TIP_engine.x = 350
    TIP_engine.y = 350
    let perc = new Network([1, 0], [4, 4, 2])
//10000000
    for (let t = 0; t < 10000000; t++) {
        let r1 = Math.random()
        let r2 = Math.random()
    ...
    function main() {
        ...
        let inputs = [TIP_engine.x / 700, TIP_engine.y / 700]
        let goals = [TIP_engine.x / 700, TIP_engine.y / 700]
        perc.compute(inputs)
        // perc.calculateDeltasSigmoid(goals)
        // perc.adjustWeights()
        let circle = new Circle(perc.outputs[0] * 700, perc.outputs[1] * 700, 3, "red")
